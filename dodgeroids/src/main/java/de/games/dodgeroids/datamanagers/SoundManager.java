...
public class SoundManager {
    ...
    /**
     * Requests the instance of the SoundManager and creates it if it does not exist. Returns a
     * single instance of the SoundManager.
     */
    ...
    public void addSound(final int soundResource) {
        soundPoolResources.put(soundResource, soundPool.load(activity, soundResource, 1));
    }

    private void loadSounds(final Activity activity) {
        // TODO: maybe an exception needs to be caught here in case the resources could not be
        // loaded
        ...
        for (int i = 0; i < length; i++) {
            ...
            soundPoolResources.put(soundID, soundPool.load(activity, soundID, 1));
        }
    ...
    public void playSound(final int soundID, final float speed) {
        if (soundOn) {
            ...
            soundPool.play(soundPoolResources.get(soundID), volume, volume, 1, 0, speed);
        }
    }
