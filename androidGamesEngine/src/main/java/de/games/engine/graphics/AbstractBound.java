...
public abstract class AbstractBound {
    ...
    protected boolean isCubeInSphere(
        ...
        Vector min =
                Vector.add(
                        boxBound.getMin(boxNode),
                        Vector.add(boxNode.getPos(), boxBound.getOffset()));
        Vector max =
                Vector.add(
                        boxBound.getMax(boxNode),
                        Vector.add(boxNode.getPos(), boxBound.getOffset()));
        float radius = sphereBound.getRadius(sphereNode);
        return (new Vector(min.x, min.y, min.z).distance(sphereNode.getPos()) < radius
                && new Vector(max.x, min.y, min.z).distance(sphereNode.getPos()) < radius
                && new Vector(min.x, max.y, min.z).distance(sphereNode.getPos()) < radius
                && new Vector(max.x, max.y, min.z).distance(sphereNode.getPos()) < radius
                && new Vector(min.x, min.y, max.z).distance(sphereNode.getPos()) < radius
                && new Vector(max.x, min.y, max.z).distance(sphereNode.getPos()) < radius
                && new Vector(min.x, max.y, max.z).distance(sphereNode.getPos()) < radius
                && new Vector(max.x, max.y, max.z).distance(sphereNode.getPos()) < radius);
    }

    protected boolean isSphereInCube(
            final Node sphereNode,
            final SphereBound sphereBound,
            final Node boxNode,
            final BoxBound boxBound) {
    ...
    protected boolean isSphereOutsideCube(
            ...
            final Node boxNode,
            final BoxBound boxBound) {
        Vector posSphere = Vector.add(sphereNode.getPos(), sphereBound.getOffset());
        Vector minBox =
                Vector.add(
                        boxBound.getMin(boxNode),
                        Vector.add(boxNode.getPos(), boxBound.getOffset()));
        Vector maxBox =
                Vector.add(
                        boxBound.getMax(boxNode),
                        Vector.add(boxNode.getPos(), boxBound.getOffset()));
        float radiusSquared = sphereBound.getRadius(sphereNode) * sphereBound.getRadius(sphereNode);
    ...
    public abstract boolean isCollidingWith(
            Node myNode,
            DetectionMethod myType,
            Node otherNode,
            DetectionMethod otherType,
