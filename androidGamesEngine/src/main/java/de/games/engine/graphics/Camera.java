...
public class Camera {
	...
	public Camera(final Vector position, final Vector direction,
			final Vector up, final float width, final float height,
			final float fovY, final float zNear, final float zFar) {
	...
	public void render(final GL11 gl) {
		GLU.gluLookAt(gl, position.x, position.y, position.z, direction.x,
				direction.y, direction.z, up.x, up.y, up.z);
	}
