...
public class BoxBound extends AbstractBound {
    ...
    public Vector getMin(final Node node) {
        return new Vector(
                min.x * node.getScale().x, min.y * node.getScale().y, min.z * node.getScale().z);
    }

    public Vector getMax(final Node node) {
        return new Vector(
                max.x * node.getScale().x, max.y * node.getScale().y, max.z * node.getScale().z);
    }
            ...
            case INNER:
                switch (otherType) {
                    ...
                    case OUTER: // pruefen ob otherNode in myNode drin ist
                        Vector myMin =
                                Vector.add(
                                        getMin(myNode), Vector.add(myNode.getPos(), getOffset()));
                        Vector myMax =
                                Vector.add(
                                        getMax(myNode), Vector.add(myNode.getPos(), getOffset()));
                        Vector otherMin =
                                Vector.add(
                                        otherBound.getMin(otherNode),
                                        Vector.add(otherNode.getPos(), otherBound.getOffset()));
                        Vector otherMax =
                                Vector.add(
                                        otherBound.getMax(otherNode),
                                        Vector.add(otherNode.getPos(), otherBound.getOffset()));

                        return (otherMin.x > myMax.x
                                || otherMax.x < myMin.x
                                || otherMin.y > myMax.y
                                || otherMax.y < myMin.y
                                || otherMin.z > myMax.z
            ...
            case OUTER:
                switch (otherType) {
                    ...
                    case OUTER: // pruefen ob sie weg von einander sind
                        Vector myHalfWidths =
                                Vector.mul(Vector.add(getMin(myNode), getMax(myNode)), 0.5f);
                        Vector otherHalfWidths =
                                Vector.mul(
                                        Vector.add(
                                                otherBound.getMin(otherNode),
                                                otherBound.getMax(otherNode)),
                        ...
                        Vector otherPos = Vector.add(otherNode.getPos(), otherBound.getOffset());

                        if (Math.abs(myPos.x - otherPos.x) <= myHalfWidths.x + otherHalfWidths.x) {
                            if (Math.abs(myPos.y - otherPos.y)
                                    <= myHalfWidths.y + otherHalfWidths.y) {
                                if (Math.abs(myPos.z - otherPos.z)
                                        <= myHalfWidths.z + otherHalfWidths.z) {
                                    return true;
                                }
                            }
                        }
            ...
            case OUTER:
                switch (otherType) {
                    ...
                    case OUTER: // Ich bin Kreis und darf einen Wuerfel nicht beruehren.
                        return this.isSphereOutsideCube(otherNode, otherBound, myNode, this);
                }
                break;
        }
        throw new RuntimeException("Impossible comparison");
