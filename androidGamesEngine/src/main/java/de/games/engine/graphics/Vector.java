...
public class Vector {
    ...
    public static Vector orthogonalProjection3D(final Vector v1, final Vector v2, final Vector x) {
        Vector vGreater;
        Vector vSmaller;
        ...
        Vector directionVector =
                new Vector(
                        vGreater.x - vSmaller.x, vGreater.y - vSmaller.y, vGreater.z - vSmaller.z);
        float scalar =
                Vector.dot(x, directionVector) / Vector.dot(directionVector, directionVector);

        return new Vector(
                scalar * directionVector.x + vSmaller.x,
                scalar * directionVector.y + vSmaller.y,
                scalar * directionVector.z + vSmaller.z);
    }
