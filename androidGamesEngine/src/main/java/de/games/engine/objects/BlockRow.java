...
public class BlockRow {
	...
	//	private int amountOfFilledBlockSlotsInRow; // amount of actual blocks in the row; without the obstacles
	...
	private Direction direction; // flag which informs if row free space went up, stayed the same or went down one in relation to the previous one
	 ...
	 * ctors
	 */
	public BlockRow(final BlockArray parent, final BlockRow previous,
			//			final int amountOfFilledBlockSlotsInRow,
			final int amountOfBottomBlocks, final int amountOfTopBlocks,
	...
	public Block[] calculateNewBlockRow() {
		...
		if (previous != null) {
			...
			case UP:
				if (previous.getAmountOfTopBlocks() == parent
						.getMinLimitOfBlocksOfASide()) { // if previous block row went up with the free space and has already reached the min limit it can has regarding the top
					direction = (Math.random() < 0.75f) ? Direction.DOWN
							: Direction.STAYED_SAME;
					// TODO hier kann man den shit resetten auf irgendne posi
				} else {
					direction = (Math.random() < 0.9f) ? Direction.UP : ((Math
							.random() < 0.5f) ? Direction.STAYED_SAME
							: Direction.DOWN);
				}
				break;
			case DOWN:
				if (previous.getAmountOfTopBlocks() == parent
						.getMaxLimitOfBlocksOfASide()) {
					direction = (Math.random() < 0.75f) ? Direction.UP
							: Direction.STAYED_SAME;
				} else {
					direction = (Math.random() < 0.9f) ? Direction.DOWN
							: ((Math.random() < 0.5f) ? Direction.STAYED_SAME
									: Direction.UP);
				}
				break;
			case STAYED_SAME:
				if (previous.getAmountOfTopBlocks() == parent
						.getMinLimitOfBlocksOfASide()) {
					direction = (Math.random() < 0.75f) ? Direction.DOWN
							: Direction.STAYED_SAME;
				} else if (previous.getAmountOfTopBlocks() == parent
						.getMaxLimitOfBlocksOfASide()) {
					direction = (Math.random() < 0.75f) ? Direction.UP
							: Direction.STAYED_SAME;
				} else {
					direction = (Math.random() < 0.5f) ? Direction.UP
							: Direction.DOWN;
				}
				break;
			}
