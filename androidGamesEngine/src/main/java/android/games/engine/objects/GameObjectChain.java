...
public final class GameObjectChain<T extends AbstractGameObject> {
	...
	public GameObjectChain(
			...
			final float minRadius, final Scene scene,
			final AbstractLevelFactory levelFactory, final int amountOfObjects,
			final float zPosOfFirstObject, final float distanceToNextObject,
			final Vector deathThresholdMin, final Vector deathThresholdMax,
		...
		for (int i = 0; i < amountOfObjects; ++i) {
			addGameObjectToChain((T) levelFactory.createGameObject(genericType,
					createStartPosition(), minRadius));
		}
	}
	...
	public int update(final float delta) {
		...
		while (it.hasNext()) {
			gameObject = it.next();
			if (!gameObject.isWithinPositionThreshold(deathThresholdMin,
					deathThresholdMax)) {
				// &&
		...
		for (int i = 0; i < removedCounter; i++) {
			gameObject = (T) levelFactory.createGameObject(genericType,
					createStartPosition(), minRadius);
	...
	public Vector createStartPosition() {
		...
		return new Vector(0.0f, 0.0f,
				(gameObjects.isEmpty() ? zPosOfFirstObject
						: (gameObjects.get(gameObjects.size() - 1)
								.getPosition().z - distanceToNextObject)));
	}
