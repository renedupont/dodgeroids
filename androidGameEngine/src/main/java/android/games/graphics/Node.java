...
public class Node {
	...
	public void setPosition(final Node n, final float x, final float y,
			final float z) {
	...
	protected Vector toLocal(final Node n, final float dx, final float dy,
			final float dz) {
		...
		float x = (float) (dx * Math.cos(yRot) + dy * Math.sin(xRot)
				* Math.sin(yRot) - dz * Math.cos(xRot) * Math.sin(yRot));
		float y = (float) (+dy * Math.cos(xRot) + dz * Math.sin(xRot));
		float z = (float) (dx * Math.sin(yRot) - dy * Math.sin(xRot)
				* Math.cos(yRot) + dz * Math.cos(xRot) * Math.cos(yRot));
